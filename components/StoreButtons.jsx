...
export default function StoreButtons({ layout = "row" }) {
    ...
    <>
      <div className={`store-buttons store-buttons--${layout}`}>
        ...
        <button
          ...
          aria-label="Join the iOS waitlist — get notified when the App Store version is ready"
        >
          <svg viewBox="0 0 24 24" aria-hidden="true" className="store-btn__icon">
          ...
          <span className="store-btn__text">
            <small>Notify me on</small>
            <strong>App Store</strong>
          </span>
        </button>
      </div>

      <IosWaitlistDialog open={iosOpen} onClose={() => setIosOpen(false)} />
    </>
  );
}
